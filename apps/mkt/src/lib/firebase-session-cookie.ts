...
import type { SessionClaims } from "@shina/billing-platform/claims";

// Mirrors apps/web/src/lib/firebase-session-cookie.ts exactly (same
// Firebase project, same cookie name/domain — see the cross-subdomain
// cookie-domain logic in app/api/auth/firebase/session/route.ts — so a
// session minted on either app is readable by the other). Duplicated
// rather than imported: each app owns its own server-only lib/, same
// pattern already used for lib/supabase/*.ts.
...
// tokens — verifiable only via the identitytoolkit "relyingparty" endpoint
// below (a {kid: pemCert} map). Using `jose` here (not firebase-admin)
// because firebase-admin depends on Node's `crypto` module and does not run
// in Next.js's Edge middleware runtime.
...
// middleware needs the raw SessionClaims shape @shina/billing-platform/
// claims's decodeSessionClaims() already produces for the Supabase path, to
// keep the subscription gate provider-agnostic without rewriting it.
