...
// Bulk-material-transport's (areia, pedra, brita) own dispatch runtime —
// same "wave 2" treatment as towing-service.ts and water-tank-service.ts.
// A bulk material service request IS an operations row
// (type="bulk_material_service_request"): same resource_id (operator) +
...
// but bulk material has two possible units — cubic meters OR tons — so the
// caller picks whichever matchVehiclesByCapacityField field the request
// actually asked for (capacityCubicMeters / capacityTons), both already
// supported by that one generic matcher.

export class BulkMaterialConflictError extends Error {
  constructor(
...
export type BulkMaterialCapacityUnit = "cubic_meters" | "tons";

export interface CreateBulkMaterialServiceRequestInput {
  tenantId: string;
  truckAssetId: string;
  ...
  quantityUnit?: BulkMaterialCapacityUnit | null;
  description?: string | null;
  customerOrganizationId?: string | null;
}

export interface BulkMaterialServiceRequest {
  id: string;
  tenantId: string;
  branchId: string;
...
function fromRow(r: Row): BulkMaterialServiceRequest {
  return {
    id: r.id,
...
/**
 * Creates a bulk-material Service Request. Same app-level pre-check +
 * DB-level GiST exclusion race guard as every other dispatch-based
 * operation's own request creator.
 */
export async function createBulkMaterialServiceRequest(
  db: SupabaseClient,
  input: CreateBulkMaterialServiceRequestInput,
): Promise<BulkMaterialServiceRequest> {
  ...
  if (!asset) throw new Error("bulk material truck asset not found");
  ...
  if (vehicleConflicts.length > 0) {
    throw new BulkMaterialConflictError(vehicleConflicts, "vehicle");
  }
  ...
  if (input.operatorResourceId) {
    ...
    if (operatorConflicts.length > 0) {
      throw new BulkMaterialConflictError(operatorConflicts, "operator");
    }
  }
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      type: "bulk_material_service_request",
    ...
    .single();
  if (error) {
    if (error.code === "23P01") throw new BulkMaterialConflictError([], "vehicle");
    throw error;
  }
  if (!data) throw new Error("failed to create bulk material service request");
