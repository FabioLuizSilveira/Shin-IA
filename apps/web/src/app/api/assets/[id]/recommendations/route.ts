...
// GET /api/assets/:id/recommendations (Etapa 7, P1) — (re)computes drafts
// from the same signals as health-score/anomalies, then upserts them with
// `ignoreDuplicates` on (tenant_id, dedupe_key): a recompute never spawns
// a duplicate of a recommendation for the same underlying signal, and a
// past human accept/dismiss decision is never silently overwritten.
// Returns every non-superseded recommendation for the asset, most recent
// first, so the UI can show pending ones alongside past decisions.
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const healthScoreInput = deriveHealthScoreInputs({
    now,
    plansDue: plansDue.map((p) => ({ result: p.result })),
    orders: healthOrders,
  });
  const healthScore = computeAssetHealthScore(healthScoreInput);
  const anomalies = detectAssetAnomalies(anomalyOrders);
  const drafts = deriveRecommendations({ healthScore, anomalies, plansDue });

  if (drafts.length > 0) {
    const { error: upsertError } = await scope.db.from("maintenance_recommendations").upsert(
      drafts.map((d) => ({
  ...
  const { data: recommendations, error: listError } = await scope.db
  ...
  if (listError) return internalError(listError);
