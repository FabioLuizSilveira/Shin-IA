...
export default function TenantStudioPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [rolesRes, permsRes] = await Promise.all([
        fetch("/api/tenant-settings/roles"),
        fetch("/api/tenant-settings/permissions"),
      ]);
      const rolesJson = (await rolesRes.json()) as { data?: TenantRole[] };
      const permsJson = (await permsRes.json()) as { data?: TenantPermission[] };
  ...
  const loadGranted = useCallback(async (roleId: string) => {
    const res = await fetch(`/api/tenant-settings/roles/${roleId}/permissions`);
  ...
  async function handleTogglePermission(permissionId: string, grant: boolean) {
    ...
    try {
      await fetch(`/api/tenant-settings/roles/${selectedRoleId}/permissions`, {
        method: "POST",
  ...
  async function handleCreateRole(e: React.FormEvent) {
    ...
    setFormError(null);
    try {
      const res = await fetch("/api/tenant-settings/roles", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function handleCreatePermission(e: React.FormEvent) {
    ...
    setFormError(null);
    try {
      const res = await fetch("/api/tenant-settings/permissions", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <AppShell title="Controle de Acesso">
      <SectionHeader
        ...
        description="Controle de acesso (IAM) dos usuários do seu tenant."
      />
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            {roles.length === 0 ? (
              <p className="text-sm text-slate-500 text-center py-8">Nenhum papel cadastrado.</p>
            ) : (
            ...
            >
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">
                  Chave (opcional, ex: gerente_operacoes)
                </label>
                <input
                  value={roleKey}
