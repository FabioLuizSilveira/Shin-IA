-- Platform x Tenant master contract — real text (was a placeholder since
-- 20260072000000_commercial_flow.sql). Provided by the user as a .docx on
-- 2026-09-10, stored verbatim. Decisions confirmed with the user:
--   * SHINÃ-side [PREENCHER] fields (CNPJ, sede, foro) kept as-is — to be
--     filled by legal before this is treated as executed.
--   * material_change = true — the one tenant that accepted the placeholder
--     v1 will be prompted to re-accept the real contract on next login
--     (custom_access_token_hook's platform_contract_current gate).
-- The document itself carries a NOTE that it is a B2B model-base requiring
-- review by a qualified lawyer before production use.
update contract_versions
  set status = 'superseded'
  where id = 'c1000000-0000-0000-0000-000000000001' and status = 'published';
...
Os níveis abaixo devem ser preenchidos no Pedido Comercial antes do uso em produção. O SLA pode variar por plano.

Disponibilidade mensal: [PREENCHER]
P1 - Crítica: ex. indisponibilidade geral | 1ª resposta: [PREENCHER] | atualização: [PREENCHER]
P2 - Alta: ex. módulo essencial indisponível | 1ª resposta: [PREENCHER] | atualização: [PREENCHER]
P3 - Média: ex. função com workaround | 1ª resposta: [PREENCHER] | atualização: [PREENCHER]
P4 - Baixa: ex. dúvida/melhoria | 1ª resposta: [PREENCHER] | atualização: [PREENCHER]
RPO: [PREENCHER]
RTO: [PREENCHER]
Horário de suporte: [PREENCHER]
Manutenção programada: [PREENCHER]
Créditos de serviço: [PREENCHER / NÃO APLICÁVEL]
...
insert into contract_versions
  (id, contract_template_id, version, title, content, content_hash, material_change, effective_at, published_at, status)
select
  gen_random_uuid(),
...
where not exists (
  ...
  where contract_template_id = 'c0000000-0000-0000-0000-000000000001' and version = 2
);
