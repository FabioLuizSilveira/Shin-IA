-- Platform-tenant master contract v4.
--
-- Fills SHINÃ's own [PREENCHER] fields (CNPJ, sede, foro) with the real,
-- now-confirmed data -- the one gap v2/v3 deliberately left open ("real
-- facts, not guessable, left untouched pending the actual data"). Everything
-- else stays byte-for-byte identical to v3 (SLA still "(a validar)",
-- TENANT-side placeholders still filled per-tenant by the composition step,
-- signatory Nome/Cargo still blank pending an actual person). Not a material
-- change -- it completes SHINÃ's own static legal identity, it does not
-- change any TENANT right or obligation -- so existing acceptances of v3
-- are NOT forced to reaccept (see resolveContractExecutionMode /
-- middleware.ts's material-change gate).

update contract_versions set status = 'superseded'
where contract_template_id = 'c0000000-0000-0000-0000-000000000001' and status = 'published';
...
Pelo presente instrumento particular, de um lado, SHINÃ INOVAÇÕES EM IA LTDA., CNPJ nº 69.125.841/0001-21, com sede na Estrada Laramara, nº 1550, Corumbá, Terra Preta, Mairiporá/SP, CEP 07666-280, doravante "SHINÃ" ou "CONTRATADA"; e, de outro, [RAZÃO SOCIAL DO TENANT], CNPJ nº [PREENCHER], com sede em [PREENCHER], doravante "TENANT" ou "CONTRATANTE"; em conjunto, "Partes", celebram este Contrato-Mestre.
...
20.2. Fica eleito o foro da Comarca de Mairiporá/SP, ressalvadas competências legais inderrogáveis.
...
select
  gen_random_uuid(), 'c0000000-0000-0000-0000-000000000001', 4,
  'Contrato-Mestre de Acesso e Uso da Plataforma Shinã',
  v.body, encode(sha256(v.body::bytea), 'hex'), false, now(), now(), 'published'
from v
where not exists (
  select 1 from contract_versions
  where contract_template_id = 'c0000000-0000-0000-0000-000000000001' and version = 4
);
