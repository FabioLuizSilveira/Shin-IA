-- Applies supabase/seed.sql (demo tenant "Acme Logística" + sample data) to
-- the hosted project. Was never run against it — seed.sql only runs
-- automatically against a local `supabase start` stack, which this project
-- no longer uses (see docs/MIGRATION_GUIDE.md, hosted-project-first
-- workflow). Content is identical to seed.sql, idempotent via
